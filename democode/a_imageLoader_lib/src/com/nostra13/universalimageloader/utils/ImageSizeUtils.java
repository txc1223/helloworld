...
public final class ImageSizeUtils {
	...
	public static int computeImageSampleSize(ImageSize srcSize, ImageSize targetSize, ViewScaleType viewScaleType, boolean powerOf2Scale) {
		...
		float targetWidth = targetSize.getWidth() / myScale;
		// int targetHeight = targetSize.getHeight();
		powerOf2Scale = false;
		Log.i("txc", "srcWidth=" + srcWidth);
		Log.i("txc", "targetWidth=" + targetWidth);
		// Log.i("txc", "srcHeight=" + srcHeight);
		float scale = 1;
		...
		if ((int) scale == 1 && (srcWidth * 3 / 4) > targetWidth) {
			scale = 2;
		}

		Log.i("txc", "scale=" + scale);
		Log.i("txc", "srcWidth /  Math.round(scale)=" + srcWidth / Math.round(scale));

		if (Math.round(scale) >= 5 && (srcWidth / Math.round(scale) > targetWidth)) {
			scale = scale + 1;
		}
		Log.i("txc", "scale=" + Math.round(scale));
		return Math.round(scale);
	}

	/**
	 * Computes scale of target size (<b>targetSize</b>) to source size
	 * (<b>srcSize</b>).<br />
	 * <br />
	 * <b>Examples:</b><br />
	 * <p/>
	 ...
	 *            view
	 * @param stretch
	 *            Whether source size should be stretched if target size is
	 *            larger than source size. If <b>false</b> then result scale
	 *            value can't be greater than 1.
