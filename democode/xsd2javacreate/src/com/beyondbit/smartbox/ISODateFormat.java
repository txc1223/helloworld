...
public class ISODateFormat extends DateFormat {
	...
	private static final TimeZone TIMEZONE_GMT = TimeZone.getTimeZone(GMT_ID + "+8:00");
			...
			do {
				...
				for (int i = 0; i < 7; i++) {
					if (source.charAt(offset) >= '0' && source.charAt(offset) <= '9') {
						millStr += source.substring(offset, offset += 1);
						// /milliseconds = parseInt(source, offset, offset +=
						// 3);
				...
				if ("".equals(millStr)) {
					...
					milliseconds = Integer.parseInt(millStr);
				}
	...
	private static void padInt(StringBuffer buffer, int value, int length) {
		...
		System.out.println(strValue + "-------padInt" + ",buffer==" + buffer + ",value=" + value + ",length=" + length);
	}
}
